"use client";

import { useState } from "react";
import { BPSData } from "@/lib/bps-utils";
import ChartBPSEnhanced from "@/components/chart-BPS-enhanced";
import { EDGE_TO_EDGE_CARD_BASE } from "@/components/marketcap/layout";

interface BPSChartWithPeriodSwitcherProps {
    initialData: BPSData[];
}

export default function BPSChartWithPeriodSwitcher({ initialData }: BPSChartWithPeriodSwitcherProps) {
    const [selectedPeriod, setSelectedPeriod] = useState<'1Y' | '5Y' | '10Y' | '20Y'>('1Y');

    const periods = [
        { key: '1Y', label: '년간', description: '년별 평균' },
        { key: '5Y', label: '5년', description: '5년 평균' },
        { key: '10Y', label: '10년', description: '10년 평균' },
        { key: '20Y', label: '20년', description: '20년 평균' },
    ];

    return (
        <div className="space-y-4">
            {/* Period Switcher */}
            <div className="flex flex-wrap gap-2 p-4 bg-muted/30 rounded-lg">
                <div className="flex items-center gap-2 text-sm font-medium text-muted-foreground">
                    <span>기간 선택:</span>
                </div>
                <div className="flex gap-1">
                    {periods.map((period) => (
                        <button
                            key={period.key}
                            onClick={() => setSelectedPeriod(period.key as any)}
                            className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${selectedPeriod === period.key
                                ? 'bg-primary text-primary-foreground shadow-sm'
                                : 'bg-background text-muted-foreground hover:bg-accent hover:text-accent-foreground border border-border'
                                }`}
                        >
                            {period.label}
                        </button>
                    ))}
                </div>
            </div>

            {/* Chart with selected period */}
            <div className={`${EDGE_TO_EDGE_CARD_BASE} p-2 sm:p-4`}>
                <ChartBPSEnhanced
                    data={initialData}
                    format="formatNumber"
                    formatTooltip="formatNumberTooltip"
                    period={selectedPeriod}
                />
            </div>
        </div>
    );
}
