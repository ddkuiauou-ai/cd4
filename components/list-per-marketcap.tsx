...
/**
 * PER을 간단하고 읽기 쉬운 형태로 포맷
 */
function formatPERSimple(num: number): { main: string; unit: string } {
    if (num === null || num === undefined || Number.isNaN(num))
        return { main: "-", unit: "" };
    if (num === 0)
        return { main: "0", unit: "배" };
...
function ListPERMarketcap({ data }: Props) {
            ...
            <div className="space-y-8 relative">{/* 테이블 섹션 */}
                <div className="space-y-4 relative">
                    <div className="flex items-center justify-between">
                        <div>
                            <h4 className="sr-only">연간 PER 변동</h4>
                            {/* SEO/접근성용 상세 설명 */}
                            <p className="sr-only">
                                기업의 연도별 PER 변동 추이를 분석합니다. 각 연도별 PER과 전년 대비 증감률을 포함한 상세 데이터를 제공합니다.
                            </p>
                        </div>
                    ...
                    <div className="block lg:hidden space-y-3 relative">{(showAllYears ? valueWithRate : valueWithRate.slice(0, 5)).map(({ date, value, changeRate }, index) => {
                        ...
                        const formattedValue = formatPERSimple(value);
                        ...
                        <div className="grid grid-cols-3 gap-8">
                                ...
                                <div className="bg-background border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden w-full">
                                    <Table className="w-full table-fixed">
                                        ...
                                        <caption className="sr-only">
                                            연도별 PER 변동 데이터 테이블입니다. 각 연도의 PER, 전년 대비 증감률, 특이사항을 포함합니다.
                                        </caption>
                                        <TableHeader>
                                            <TableRow className="hover:bg-transparent border-b bg-gray-50 dark:bg-gray-800">
                                                ...
                                                <TableHead className="text-right font-semibold text-gray-800 dark:text-gray-200 w-[40%]">PER</TableHead>
                            ...
                            <div className="col-span-1 space-y-4">
                                ...
                                <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-background relative">
                                    ...
                                    {/* SEO/접근성용 설명 */}
                                    <p className="sr-only">PER의 평균값, 역대 최고치, 역대 최저치 등 핵심 통계 정보입니다.</p>
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-center">
                                            <span className="text-xs text-gray-600 dark:text-gray-400">평균 PER</span>
                                            <span className="font-bold text-sm">{formatPERSimple(avgValue).main}{formatPERSimple(avgValue).unit}</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="text-xs text-gray-600 dark:text-gray-400">역대 최고</span>
                                            <span className="font-bold text-sm text-gray-800 dark:text-gray-200">{formatPERSimple(maxValue).main}{formatPERSimple(maxValue).unit}</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="text-xs text-gray-600 dark:text-gray-400">역대 최저</span>
                                            <span className="font-bold text-sm text-gray-800 dark:text-gray-200">{formatPERSimple(minValue).main}{formatPERSimple(minValue).unit}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-background relative">
                                    ...
                                    {/* SEO/접근성용 설명 */}
                                    <p className="sr-only">PER 변동 패턴을 바탕으로 한 상승/하락 연도 분포, 최고 변동률과 최대 변동률 등 분석입니다.</p>
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-center">
                                ...
                                <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-background">
                                    ...
                                    {/* SEO/접근성용 설명 */}
                                    <p className="sr-only">PER 변동 패턴을 바탕으로 한 변동성 분석과 투자 전망에 대한 인사이트입니다.</p>
                                    <div className="space-y-2 text-xs">
                                        ...
                                        <div className="p-2 bg-gray-50 dark:bg-gray-800/50 rounded">
                                            <div className="font-medium mb-1">평가 전망</div>
                                            <div className="text-gray-600 dark:text-gray-400">
                                                {averageGrowth > 5
                                                    ? "평균 변동률 5% 이상의 활발한 시장"
                                                    : averageGrowth > 0
                ...
                <div className="mt-8 space-y-6 relative lg:hidden">
                    ...
                    <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-6 bg-background relative">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600 dark:text-gray-400">평균 PER</span>
                                <span className="font-bold">{formatPERSimple(avgValue).main}{formatPERSimple(avgValue).unit}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600 dark:text-gray-400">역대 최고</span>
                                <span className="font-bold text-gray-800 dark:text-gray-200">{formatPERSimple(maxValue).main}{formatPERSimple(maxValue).unit}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600 dark:text-gray-400">역대 최저</span>
                                <span className="font-bold text-gray-800 dark:text-gray-200">{formatPERSimple(minValue).main}{formatPERSimple(minValue).unit}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-6 bg-background relative">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h4 className="font-semibold">변동 분석</h4>
                        </div>
                        <div className="space-y-4">
                            <div className="flex justify-between items-center">
...
export default ListPERMarketcap;