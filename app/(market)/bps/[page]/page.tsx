...
export async function generateStaticParams() {
    const total = await countSecurityRanks("bps");
...
interface BpsRankPageProps {
    params: Promise<{
        page: string;
    }>;
}

export async function generateMetadata({ params }: BpsRankPageProps): Promise<Metadata> {
    const temp = await params
    const page = parseInt(temp.page, 10) || 1;
    const { items, latestDate } = await getSecurityRanksPage("bps", page, 'desc');
    const topSecurityNames = items.slice(0, 5).map(s => s.korName || s.name);

    const title = `KOSPI·KOSDAQ 종목 주당 순자산(BPS) 순위 ${page}페이지 | ${siteConfig.name}`;
    const description = `${latestDate} 기준 KOSPI, KOSDAQ, KONEX 전체 종목의 주당 순자산(BPS) 순위 ${page}페이지입니다. ${topSecurityNames.join(', ')} 등 주요 종목의 현재가, 등락률, 거래소 정보를 확인하고 투자 기회를 포착하세요.`;

    return {
        title,
        description,
        keywords: ['종목 순위', '주당 순자산(BPS) 랭킹', '코스피', '코스닥', '코넥스', '주식 투자', '가치 투자', '천하제일 단타대회', ...topSecurityNames],
        alternates: {
            canonical: `${siteConfig.url}/bps/${page}`,
        },
        openGraph: {
            title,
            description,
            url: `${siteConfig.url}/bps/${page}`,
...
async function BpsRankPage({ params }: BpsRankPageProps) {
