/**
 * Utility functions for PBR (Price-to-Book Ratio) calculations and data processing
 */
...
export interface PBRPeriodAnalysis {
    periods: Array<{
        label: string;
        value: number;
        desc: string;
...
/**
 * Calculate period analysis for PBR data
 */
export function calculatePBRPeriodAnalysis(
    result: PBRData[],
    displayName: string,
    market: string
): PBRPeriodAnalysis | null {
    ...
    // 기간별 평균 계산
    const periods = [
        { label: '최근 PBR', months: 0, desc: '현재 기준' },
...
/**
 * Aggregate PBR data by different time periods
 */
export function aggregatePBRDataByPeriod(data: PBRData[], period: PeriodType): PeriodData[] {
 ...
 */
function aggregateDailyData(data: PBRData[]): PeriodData[] {
 ...
 */
function aggregateWeeklyData(data: PBRData[]): PeriodData[] {
 ...
 */
function aggregateMonthlyData(data: PBRData[]): PeriodData[] {
 ...
 */
function aggregateYearlyData(data: PBRData[]): PeriodData[] {
    const sortedData = data.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
